function updated_xray_processing_gui()
 ...
 function edgeDetection(~, ~)
    % Initialize variables
    max_intensity = max(denoised_img(:)); % Maximum intensity value in the denoised image
    threshold = 0.5 * max_intensity; % Set a threshold for considering high-intensity edges
    gray_img = rgb2gray(denoised_img); % Convert the denoised image to grayscale
    edged_img  = edge(gray_img, 'Canny', 0.4, 2); % Adjust parameters as needed

    % Apply threshold to highlight high-intensity edges
    edged_img = edged_img & (denoised_img >= threshold);

    % Convert the binary image to grayscale for display
    edged_img_gray = uint8(edged_img) * 255;

    % Update displayed image with optimal edges
    cla(ax3);
    axes(ax3);
    imshow(edged_img_gray);
    title(ax3, 'Edge Detected Image (High-Intensity)');
    axis(ax3, 'equal');

    % Draw green lines over detected edges
    green_lines = drawEdges(edged_img);
end
   ...
   function rotateImage(~, ~)
    ...
    function green_lines = drawEdges()
        % Draw green lines over detected edges
        [row, col] = find(edges);
        axes(ax3);
        hold on;

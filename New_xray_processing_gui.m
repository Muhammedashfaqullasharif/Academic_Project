function updated3_xray_processing_gui()
    ...
    % Resize the grayscale image to increase resolution
    % Adjust the scale factor as needed
    resized_gray_img = imresize(gray_img, 2); % Example: Increase resolution by a factor of 2

    % Initialize the combined edge map
    combined_edge_map = zeros(size(resized_gray_img));
    ...
    for i = 1:numel(thresholds)
        for j = 1:numel(std_deviations)
            ...
            edge_map = edge(resized_gray_img, 'Canny', thresholds(i), std_deviations(j));
    ...
    % Resize the combined edge map back to the original size
    edge_map = imresize(combined_edge_map, size(gray_img));

    % Apply Otsu thresholding to separate strong from weak edges
    threshold = graythresh(edge_map); % Compute Otsu threshold
    edges_binary = imbinarize(edge_map, threshold); % Binarize the edge map
    ...
    title(ax_output, 'Enhanced Edge Map (Disk Size 1)');
    axis(ax_output, 'equal');

    % Create a tab for preview
    tab1 = uitab(tab_group, 'Title', 'Enhanced Edge Map (Disk Size 1)');
    ax_tab1 = axes('Parent', tab1);
    imshow(enhanced_edges, 'Parent', ax_tab1);
    title(ax_tab1, 'Enhanced Edge Map (Disk Size 1)');
end
    ...
    function calculate2DPeaks()
        ...
        % Plot intensity peaks in the preview tab
        tab_2d_peak_search = uitab(tab_group, 'Title', '2D Peak Search Preview');
        ax_2d_peak_search = axes('Parent', tab_2d_peak_search);
        plot(ax_2d_peak_search, 1:length(intensities), intensities, 'b', 'LineWidth', 2);
        hold(ax_2d_peak_search, 'on');
        plot(ax_2d_peak_search, locs, pks, 'ro', 'MarkerSize', 10);
        title(ax_2d_peak_search, 'Intensity Peaks Along Columns');
        xlabel(ax_2d_peak_search, 'Column Index');
        ylabel(ax_2d_peak_search, 'Intensity');
        legend(ax_2d_peak_search, {'Intensity', 'Peaks'});
        hold(ax_2d_peak_search, 'off');
    end
    ...
    function calculate3DPeaks()
        ...
        % Plot 3D intensity peaks in the preview tab
        tab_3d_peak_search = uitab(tab_group, 'Title', '3D Peak Search Preview');
        ax_3d_peak_search = axes('Parent', tab_3d_peak_search);
        scatter3(ax_3d_peak_search, X, Y, intensities, 50, colors, 'filled');
        title(ax_3d_peak_search, '3D Intensity Peaks');
        xlabel(ax_3d_peak_search, 'Column Index');
        ylabel(ax_3d_peak_search, 'Row Index');
        zlabel(ax_3d_peak_search, 'Intensity');
        colormap(ax_3d_peak_search, cmap);
        c = colorbar(ax_3d_peak_search);
        c.Label.String = 'Intensity';
    end

    function drawBoundaries(~, ~)
    % Check if enhanced edges and peak locations are available
    if isempty(enhanced_edges) || isempty(peak_locations)
        disp('Please perform edge detection and peak search first.');
        return;
    end
    ...
    [B, ~] = bwboundaries(enhanced_edges, 'noholes');
    ...
    % Add intensity peak locations to outer boundaries
    for i = 1:length(peak_locations)
        % Convert peak location to pixel coordinates
        peak_x = peak_locations(i);
        peak_y = round(size(boundaries_img, 1) / 2); % Assuming peak location is at the center of the image
        
        % Add peak location to outer boundaries
        outer_boundaries{end + 1} = [peak_y, peak_x];
    end

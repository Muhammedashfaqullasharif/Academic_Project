function updated2_xray_processing_gui()
    ...
    % Shared variables
    smoothed_img = [];
    enhanced_edges = []; % Shared variable for enhanced edges
    peak_locations = []; % Shared variable for peak locations
    intensity_surface = []; % Initialize intensity surface
    ...
    btn_boundary = uicontrol('Parent', btn_panel, 'Style', 'pushbutton', 'String', 'Boundary', ...
    ...
    function performEdgeDetection(~, ~)
        ...
        % Call edge detection function with smoothed image
    ...
    function performEdgeDetectionInternal()
        ...
        % Enhance edges using morphological operations with disk
        % structuring element of size 3
        se = strel('disk', 3); % Size 3
        enhanced_edges = imclose(edges_binary, se); % Close gaps in edges
    ...
    function rotateImage(~, ~)
        ...
        bar(ax_tab4, histogram_edges(1:end-1), histogram_values); % Adjusted to have the same length
    ...
    function drawBoundaries(~, ~)
        ...
        % Find boundaries in the enhanced edge map
        boundaries = bwboundaries(enhanced_edges);
        
        % Create a new tab for preview
        tab_boundary = uitab(tab_group, 'Title', 'Boundaries');
        ax_tab_boundary = axes('Parent', tab_boundary);
        
        % Plot the original image
        imshow(img, 'Parent', ax_tab_boundary);
        hold on;
        
        % Draw boundaries as thin red lines
        for i = 1:length(boundaries)
            plot(ax_tab_boundary, boundaries{i}(:, 2), boundaries{i}(:, 1), 'r', 'LineWidth', 1);
        end
        
        % Set title and hold off
        title(ax_tab_boundary, 'Boundaries with Thin Red Lines');
        hold off;
    end
